...
当`npx skills add alchaincyf/nuwa-skill`因网络连接问题失败时（特别是git clone操作超时），可以使用GitHub API手动下载技能文件：

1. **创建目录结构**：
```bash
mkdir -p ~/.hermes/skills/software-development/huashu-nuwa/{references,scripts,examples}
```

2. **通过API下载核心文件**：
```bash
# 下载SKILL.md
curl -L "https://api.github.com/repos/alchaincyf/nuwa-skill/contents/SKILL.md" | jq -r .content | base64 --decode > ~/.hermes/skills/software-development/huashu-nuwa/SKILL.md

# 下载参考文件
curl -L "https://api.github.com/repos/alchaincyf/nuwa-skill/contents/references/skill-template.md" | jq -r .content | base64 --decode > ~/.hermes/skills/software-development/huashu-nuwa/references/skill-template.md
curl -L "https://api.github.com/repos/alchaincyf/nuwa-skill/contents/references/extraction-framework.md" | jq -r .content | base64 --decode > ~/.hermes/skills/software-development/huashu-nuwa/references/extraction-framework.md

# 下载脚本文件
for script in merge_research.py quality_check.py srt_to_transcript.py download_subtitles.sh; do
  curl -L "https://api.github.com/repos/alchaincyf/nuwa-skill/contents/scripts/$script" | jq -r .content | base64 --decode > ~/.hermes/skills/software-development/huashu-nuwa/scripts/$script
  chmod +x ~/.hermes/skills/software-development/huashu-nuwa/scripts/$script
done
```

3. **验证安装**：
```bash
ls -la ~/.hermes/skills/software-development/huashu-nuwa/
```

这种方法适用于GitHub连接不稳定但API访问正常的环境。下载后，技能即可正常使用。
