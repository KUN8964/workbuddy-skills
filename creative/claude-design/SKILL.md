...
5. **Build the artifact**
   ...
   - For dark theme conversions of existing light artifacts, see `references/dark-theme-redesign.md` for the full workflow, CSS variable swap pattern, badge adaptation, and pitfalls.
...
## Pixel-Sampling Design Extraction (Vision Fallback)

When the current model does NOT support vision (e.g. DeepSeek, some API providers), you cannot feed design screenshots directly to the LLM for analysis. Use **programmatic pixel sampling** with Python + PIL to extract the design system from PNG/JPEG mockups.

### Workflow

1. **Check image dimensions** — design screenshots are often 2x retina (e.g. 3640px wide → real design width ~1820px). Use `Image.open()` and `img.size` to get pixel dimensions.

2. **Extract brand palette** — sample pixels on a grid pattern (every 20-30px), skip near-white and near-black, count the most frequent remaining colors to identify brand accent colors:

```python
from PIL import Image
import collections
img = Image.open("mockup.png").convert("RGB")
w, h = img.size
pixels = []
for x in range(0, w, 50):
    for y in range(0, h, 50):
        ...
        if not (r > 240 and g > 240 and b > 240) and not (r < 20 and g < 20 and b < 20):
            pixels.append((r, g, b))
collections.Counter(pixels).most_common(15)
```

3. **Analyze specific regions** — crop regions of interest (navigation, hero, footer, feature cards) and sample their pixel colors separately.

4. **Horizontal section analysis** — scan the page top-to-bottom in bands (e.g. every 100px), compute average color per band, label each band (white / dark_blue / gray / light_blue / dark), and track when the color changes to identify distinct content sections and their order.

5. **Count brand color usage percentage** — measure how much of each page is covered by brand accent colors to understand visual hierarchy.

### Pitfalls

- **zip encoding**: Chinese filenames in zip files may be garbled with system `unzip`. Prefer Python `zipfile` module.
- **Retina scaling**: Image dimensions are often 2x. Divide by 2 to get the actual design width — but do NOT halve the HTML output, just set max-width accordingly.
- **PIL may need install**: `pip install Pillow` first.
- **Sampling only gives palette, not layout**: you still need to infer section order from the color band analysis and filenames (e.g. `00-首页.png`, `01-产品-xxx.png` encode page hierarchy).
- **Format check**: use `file` command or `img.format` to confirm PNG/JPEG before pixel operations.

## Multi-Page SPA Assembly (Avoiding Sub-Agent Limits)

When consolidating 10+ HTML pages into a single SPA file, sub-agents often hit output limits (files >60KB get truncated). Use this strategy instead:

1. **Extract body content locally first** — use `execute_code` with `open()` on each sub-page, regex-extract `<body>...</body>` content, strip `<nav>` and `<footer>`, store in a dict.

2. **Build the SPA in Python** — write the assembly script in `execute_code`, constructing the full HTML string programmatically. Use f-strings to embed extracted page content into template slots. This sidesteps sub-agent truncation.

3. **Verify with browser tools** — after writing, navigate to the file and check `browser_console` for zero JS errors. Test navigation by dispatching click events or directly calling the navigate function from console.

### JS Event Delegation Pattern (Proven)

```html
<script>
document.addEventListener('click', function(e) {
  // Nav links with data-page attribute
  var navLink = e.target.closest('.nav-links a[data-page]');
  if (navLink) { e.preventDefault(); navigateTo(navLink.getAttribute('data-page')); return; }
  
  // Any element with data-page (cards, buttons)
  var card = e.target.closest('[data-page]');
  if (card) { e.preventDefault(); navigateTo(card.getAttribute('data-page')); return; }
  
  // Fallback: inline onclick
  var el = e.target.closest('[onclick]');
  if (el) {
    var match = el.getAttribute('onclick').match(/navigateTo\\(['"]([^'"]+)['"]\\)/);
...
Add `data-page="page-id"` to every clickable element (including inner headings, not just wrapper divs) for robust click handling.

### Pitfalls
- `write_file()` truncates content over ~60KB — use Python `open()` with `f.write()` directly in `execute_code` for large HTML files
- Sub-agent `delegate_task` with large tasks (250KB+ HTML output) will hit `max_iterations` or token caps — let the main agent build the file
- Two `<script>` blocks in the output (one from embedded page content, one from your SPA logic) cause silent JS failures — merge into one block
- `read_file()` returns cached "File unchanged" messages, not actual content — always use `open()` in `execute_code` to read files
- **CSS string replacement can fail silently**: When patching CSS via Python `str.replace()` in a large HTML file, old/new strings may not match due to invisible whitespace differences (tabs vs spaces, trailing newlines, BOM). The replace silently does nothing — file appears written but CSS rule is absent. Always verify with `search_files` (pattern matching) or `grep` after replacement. Preferred fallback: inject new CSS rules before the `</style>` tag (`html.replace('</style>', new_css + '</style>')`) rather than targeting mid-document strings. For full theme overhauls, replace the entire `<style>...</style>` block in one operation.
- **Merging two CSS design systems**: When source pages use different conventions (e.g. CSS variables + `.inner` class vs raw values + `.container` class), unify by: (a) CSS variables as global base, (b) both `.inner` and `.container` map to same `max-width` wrapper via combined selector, (c) include both class-name sets in merged stylesheet, (d) override section-level padding discrepancies. Support both systems rather than converting one into the other.
- **`browser_click` on nested elements fails**: When a snapshot ref points to a child inside a `[data-page]` anchor (e.g. `<h3>` inside `<a data-page="foo">`), `browser_click` may target the child without bubbling. Verify navigation via `browser_console` JS calls (`navigateTo('page-id')`) instead.
- **Dark theme badge adaptation**: When converting a light-themed site to dark, certification badges, logos, and footer icons with white/transparent backgrounds become invisible on black. Apply `filter: brightness(0) invert(1)` to `.cert-badge` or `.logo-img` elements, or swap to dark-mode variants if available. Verify badge visibility with `browser_console` checking `offsetWidth`/`offsetHeight` (not zero means visible).
- **Version control before major redesigns**: When the user asks for a style overhaul of an existing artifact, version it first with `cp index.html index_v1.html` in the same directory. The user may want to compare or revert. This pattern is so cheap it should be automatic.
...
- open in a browser tool and check console errors with `browser_console`
- test key interactions (navigation, accordions, dropdowns) via `browser_console` JS calls
- test light/dark or variants if present
- test responsive breakpoints if relevant
- **Vision fallback**: On providers that don't support vision (DeepSeek, some APIs), `browser_vision` will fail with `image_url` errors. Don't attempt it. Use `browser_console` to query element states, CSS computed values, and DOM structure instead. Take a screenshot with `browser_screenshot` for the user to inspect manually, but don't pipe it to vision analysis.
